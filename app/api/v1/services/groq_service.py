...
async def send_message(
    ...
    if result['found']:
        return {'source': 'faq', 'response': result['response']}

    cache_response = await cache_respost(context_user, r)

    if cache_response:
        return {'source': 'redis', 'response': cache_response}

    chat_response = await completion(client, context_user, context_system)

    if not chat_response:
        raise HTTPException(
            status_code=409, detail='Erro ao mandar mensagem ao agente.'
        )
    ...
    return {'source': 'ai', 'response': response_completed}
