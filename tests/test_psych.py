...
async def test_psych_not_found(token_fakepsych):
    ...
    assert response.status_code == status
    assert response.json()['detail'] == 'Psicólogo não encontrado! Tente realizar o login novamente' #noqa
...
async def test_availability_has_conflicts_same_time(token_psych, availability):
    ...
    assert response.status_code == status
    assert response.json()['detail'] == 'Conflito de horário detectado entre 08:30 e 10:30, confira seus horários.' #noqa


@pytest.mark.asyncio
async def test_availability_has_conflicts_existing_schedule_has_not_ended(token_psych, availability): #noqa
    payload = {
        "availabilities": [
            {
                "days_of_the_week": [
    ...
    assert response.status_code == status
    assert response.json()['detail'] == 'Conflito de horário detectado entre 09:30 e 15:30, confira seus horários.' #noqa
...
async def test_availability_has_conflicts_end_time(token_psych, availability): #noqa
    ...
    assert response.status_code == status
    assert response.json()['detail'] == 'Conflito de horário detectado entre 07:30 e 09:30, confira seus horários.' #noqa
